...
def send_open_document_request(document_path: str, update_indexes: bool = False):
    ...
    if update_indexes:
        url = f"http://localhost:{port}/open-document-and-update"
    else:
...
def open_file(document_path: str):
    ...
    send_open_document_request(document_path, update_indexes=False)

@click.command()
@click.argument("document_path")
def open_file_and_update(document_path: str):
    logging.basicConfig(level=logging.INFO)
    send_open_document_request(document_path, update_indexes=True)
